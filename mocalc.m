...
% repulsion.
guess=SCF_oneloop(out.basis,settings.method,N,M,out.T,out.Vne,out.S,out.ERI,...
    Vnn,zeros(M),1,zeros(M),settings.ExchFunctional,settings.CorrFunctional,grid);
guess.e_Etot=1;
guess.e_P=1;
while guess.e_Etot>=settings.tolEnergy || guess.e_P>=settings.tolDensity
    guess=SCF_oneloop(out.basis,settings.method,N,M,out.T,out.Vne,out.S,...
        out.ERI,Vnn,guess.P,guess.Etot,guess.Vxc,settings.ExchFunctional,...
        settings.CorrFunctional,grid);
end
out.J=guess.J;
out.K=guess.K;
