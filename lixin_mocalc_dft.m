...
% repulsion.
guess=SCF_oneloop(out.basis,settings.method,N,M,out.T,out.Vne,out.S,out.ERI,Vnn,zeros(M),0,zeros(M),settings.ExchFunctional,settings.CorrFunctional,grid);
guess.e_Etot=1;
guess.e_P=1;
while guess.e_Etot>=settings.tolEnergy || guess.e_P>=settings.tolDensity
    guess=SCF_oneloop(out.basis,settings.method,N,M,out.T,out.Vne,out.S,out.ERI,Vnn,guess.P,guess.Etot,guess.Vxc,settings.ExchFunctional,settings.CorrFunctional,grid);
end
...
function SCF=SCF_oneloop(basis,method,N,M,T,Vne,S,ERI,Vnn,P_0,Etot_0,Vxc_0,ExchFunctional,CorrFunctional,grid)
...
for q=1:M
    for p=1:M
        ...
        elseif strcmp(method,'RKS')
            F(q,p)=F(q,p)+SCF.J(q,p)+Vxc_0(q,p);
        end
    end
end
...
if strcmp(method,'RKS')
    [SCF.Vxc,SCF.Exc,SCF.rhoInt]=int_xc(basis,SCF.P,grid,ExchFunctional,CorrFunctional);
end
...
for u=1:M
    for v=1:M
        SCF.E0=SCF.E0+SCF.P(u,v)*(T(u,v)+Vne(u,v)+(1/2)*SCF.J(u,v));
        if strcmp(method,'RHF')
            SCF.E0=SCF.E0-(1/2)*SCF.K(u,v);
        end
    end
end
if strcmp(method,'RKS')
    SCF.E0=SCF.E0+SCF.Exc;
end
